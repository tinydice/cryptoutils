...
zeroEntropyHash = "e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855"
...
def get_mnemonic(entropyHash, wordCount):
    if (wordCount) == 24:
        ...
        entropyHash = entropyHash[:16]
        v = int.from_bytes(entropyHash, 'big') << 4
    ...
    if (wordCount) == 24:
        indexes[-1] |= hashlib.sha256(entropyHash).digest()[0]
    elif (wordCount == 12):
        indexes[-1] += hashlib.sha256(entropyHash).digest()[0] >> 4

    mnemonic_list = [wordList[i] for i in indexes]
